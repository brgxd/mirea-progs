...
public:
	...
	{
		MyArrayChild a(ptr, count+1);
		a.ptr[a.count - 1] = value;
		return a;
	}
	...
	MyArrayChild SubSequence(int StartIndex = 0, int Length = -1)
	{
		...
		MyArrayChild a(ptr, Length);
		for (int i = 0; i < Length; i++, StartIndex++)
			a.ptr[i] = ptr[StartIndex];
		return a;
	}
	...
	public:
		...
		int BinSearch(double value)
		{
			int l = 0;
			int r = count - 1;
			for(int middle = 0;middle > 1; middle = (r - l)/2)
			{
				if (ptr[middle] == value)return middle;
				if (ptr[middle] > value)middle = r;
				else middle = l;
			}
			return -1;
		}
